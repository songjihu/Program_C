...
{	10,9,3,4,2},
	...
	*/
	...
	//【题型五】几何问题
	//1.输入一个点的坐标，一个圆心坐标和半径长度，计算点是否在圆内 略
	
	//2.在100个点中寻找距离最近的2个点 略

	//3.在100个点中寻找距离最远的2个点 略

	//4.输入ABCD坐标，假设ABC构成三角形，判断D是否在三角形中
	/*is_in_triangle();
	*/

	//5.求100个点中组成的最大三角形面积 略

	//6.求三角形外接圆公式（圆心到三点距离相等，化简解方程）略

	//【题型六】方程问题
	//1.求ax2+bx+c=0的解（用fabs(x)<=1e-6即为x=0，正负用1e-6代表0作为分界线） 略

	//2.用二分法求2x3-4x2+3x-6=0求位于(-10,10)之间的一个根
	dichotomy();
